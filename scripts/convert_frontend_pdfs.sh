...
# Frontend System Design PDF Converter
# This script converts README.md files from frontend-system-design folders to PDFs
# It checks for existing PDFs and only recreates missing ones
# Includes automatic mermaid diagram error fixing

set -e  # Exit on any error

# Configuration
SOURCE_DIR="frontend-system-design"
OUTPUT_DIR="frontend-system-design-pdfs"
...
# Function to fix common mermaid syntax issues
fix_mermaid_syntax() {
    local file="$1"
    ...
    # Fix common mermaid issues:
    # 1. Replace special arrow characters → with regular text
    # 2. Simplify complex edge labels that might cause parsing errors
    sed -e 's/→/ to /g' \
        -e 's/-->|[^|]*Hash([^)]*)[^|]*|/ --> /g' \
        -e 's/-->|[^|]*→[^|]*|/ --> /g' \
        "$file" > "$temp_file"
    ...
    while [ $attempt_count -lt $max_attempts ]; do
        ...
        if [ $attempt_count -eq 1 ]; then
            ...
            if pandoc "$input_file" -o "$output_file" $PANDOC_OPTIONS --filter mermaid-filter 2>/dev/null; then
                ...
                print_status $YELLOW "   ⚠️  Mermaid conversion failed, trying to fix syntax..."
            fi
        fi
...
# Main execution
main() {
    print_status $BLUE "🚀 Frontend System Design PDF Converter Starting..."
    print_status $BLUE "=================================================="
    ...
    # Count total folders
    total=$(find "$SOURCE_DIR" -maxdepth 1 -type d -name "*-*" | wc -l)
    
    print_status $BLUE "\n📊 Found $total system design folders to process"
...
# Help function
show_help() {
    echo "Frontend System Design PDF Converter"
    echo "====================================="
    echo ""
    ...
    echo ""
    echo "This script:"
    echo "  • Converts README.md files from frontend-system-design/* folders to PDFs"
    echo "  • Only processes files that don't exist or are newer than existing PDFs"
    echo "  • Automatically fixes common mermaid diagram syntax issues"
...
if [ "$LIST_ONLY" = true ]; then
    echo "Frontend System Design README.md files and PDF status:"
    echo "====================================================="
    for dir in $SOURCE_DIR/*/; do
